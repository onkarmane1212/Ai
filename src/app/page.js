            ...
            {issues.local_issues.map((issue, index) => (
              <div key={`local-${index}`} className="border p-4 rounded">
                ...
                <div className="flex justify-between items-center mt-2 text-xs">
                  <span className={`px-2 py-1 rounded ${issue.sentiment === 'positive' ? 'bg-green-100 text-green-800' :
                      issue.sentiment === 'negative' ? 'bg-red-100 text-red-800' :
                        'bg-yellow-100 text-yellow-800'
                    }`}>
                    {issue.sentiment || 'neutral'}
                        ...
                        <div className="flex items-start space-x-4">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="flex items-center justify-between text-sm mb-2">
                              ...
                              <span className={`px-2 py-1 rounded-full text-xs ${leader.sentiment === 'positive' ? 'bg-green-100 text-green-800' :
                          ...
                          <div key={`negative-${index}`} className="mb-6 p-4 bg-red-50 rounded-lg border border-red-100 hover:shadow-sm transition-shadow">
                            ...
                            {item.sentiment_score && (
                              <div className="mt-2 flex items-center text-xs">
                                ...
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${item.sentiment_score < -0.5 ? 'bg-red-100 text-red-800' :
                      ...
                      <div className="space-y-4">
                        {Object.entries({
                          ...
                          <div key={location}>
                            ...
                            <div className="w-full bg-gray-100 rounded-full h-2.5">
                              <div
                                className={`h-full rounded-full ${location === 'Urban' ? 'bg-green-500' : 'bg-yellow-500'
                                  }`}
                                style={{ width: `${percentage}%` }}
                              ></div>
                            </div>
                    ...
                    {Object.entries(searchResults.platform_sentiment_comparison).map(([platform, data]) => (
                      <div key={platform} className="space-y-2">
                        <div className="flex justify-between items-center">
                          ...
                          <div className="flex items-center">
                            <span className={`text-sm font-semibold ${data.score >= 60 ? 'text-green-600' :
                      ...
                      <div className="space-y-4">
                        {searchResults.source_analysis.top_sources.map((source, index) => (
                          <div key={index} className="flex flex-wrap items-center py-3 border-b border-gray-100 last:border-0">
                            ...
                            <div className={`ml-3 px-3 py-1 text-xs rounded-full font-medium ${source.bias === 'center' ? 'bg-blue-100 text-blue-800' :
